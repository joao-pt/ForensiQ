...
- **`o plano interno de refactor`**: a taxonomia de crimes e a prioridade derivada da Política Criminal passam a estar formalizadas neste ADR; a forma dos campos da `Occurrence` referencia-o.
- **`a especificação de art direction`** (Fase 3): §Hero e `geo-hero` — colorbar/legenda passa de P1-P4 para **2 estados** (`prioritária`/`normal`); a coluna "Pri." da tabela idem.
