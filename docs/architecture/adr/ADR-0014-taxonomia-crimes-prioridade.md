...
Accepted — 2026-05-30. Formaliza o tema **T19** e as decisões de produto **D9, D10 e D11** do `docs/refactor/REFACTOR_MANIFEST.md` (§3, §6). Estende **T03** (campos da `Occurrence`) e liga-se a **T06** (feed de actividade) e **T18/D8** (`Occurrence` POST-only). Acompanha o ADR-0013 (GPS na custódia) como segundo ADR estrutural da Fase 2 do refactor.
...
O `REFACTOR_MANIFEST.md` regista em §6 as decisões **D9, D10 e D11** e desenvolve-as em **T19** (§3). Este ADR formaliza-as; não as reabre.
...
8. **Implicação para a Fase 3 (`docs/refactor/art-direction.md`).** A colorbar/legenda do hero e a coluna "Pri." passam de P1-P4 para **2 estados** (`prioritária`/`normal`). Actualizar a §Hero do `art-direction.md` e o `geo-hero` em conformidade. (Trabalho de Fase 3 — apenas registado aqui.)
...
- **`docs/refactor/REFACTOR_MANIFEST.md`**: T19 e D9/D10/D11 passam de "decisão registada" a "formalizada em ADR-0014"; T03 referencia este ADR para a forma dos campos.
- **`docs/refactor/art-direction.md`** (Fase 3): §Hero e `geo-hero` — colorbar/legenda de P1-P4 → 2 estados (`prioritária`/`normal`); coluna "Pri." da tabela idem.
...
- Cabeça actual da migration chain: `0017_alter_auditlog_options_auditlog_sequence`. O track GPS (ADR-0013) **reserva `0018`-`0019`** (rename `gps_lng` + GPS na custódia); para **evitar colisão de numeração** entre os tracks paralelos (T01/T02 e T19, REFACTOR_MANIFEST §5), este ADR usa a faixa **`0020`-`0021`**.
...
- **Pré-condição (D8/T18):** a decisão **D8** (`REFACTOR_MANIFEST.md` §6, decisão do dono 2026-05-30) torna a `OccurrenceViewSet` POST-only; executa-se em **T18**, *antes* de T19 — adicionar `http_method_names = ['get','post','head','options']` à `OccurrenceViewSet` (`core/views.py:203`, hoje `ModelViewSet` sem restrição), senão a API continua a expor PUT/PATCH/DELETE que o trigger 0013 recusa (500 em produção, mutação silenciosa em SQLite de teste). O teste `PUT/PATCH/DELETE → 405` (abaixo) verifica esta pré-condição.
...
- `docs/refactor/REFACTOR_MANIFEST.md` — §3 (T03, T19), §6 (D8, D9, D10, D11), §7 (verificação da `Occurrence` imutável vs ViewSet mutável).
