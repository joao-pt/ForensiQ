...
A credencial governa a **leitura**, com uma exceção: o papel `FORENSIC_EXPERT` tem **leitura total por função**, independentemente da credencial — um perito pode ser chamado a pronunciar-se sobre processos de outras áreas ou divisões, pelo que precisa de ver toda a prova. A leitura total é, assim, de quem é **staff**, tem credencial **`NACIONAL`** ou tem o papel **`FORENSIC_EXPERT`**. A capacidade de **alterar** estados vem da **função**: o `FORENSIC_EXPERT` tem *override* operacional de escrita (sobre os itens que vê) e pode nomear intermediários; o `CHEFE_SERVICO` é **só-leitura**. Assim, perito e chefe veem ambos tudo, mas **só o perito altera**.
...
0. papel `FORENSIC_EXPERT` (leitura total por função) ou `is_staff`;
...
6. **Perito sem credencial nacional.** Um `FORENSIC_EXPERT` tem leitura total **por função**, mesmo com `clearance=NORMAL` — a visibilidade por credencial mantém-se apenas para os restantes papéis.
...
- **Testes (acrescem às 5 famílias do ADR-0016):**
  ...
  8. *Credencial vs função* — `NACIONAL` (qualquer função) vê tudo; `FORENSIC_EXPERT` vê tudo por função mesmo com `NORMAL`; os restantes papéis `NORMAL` continuam *need-to-know*.
