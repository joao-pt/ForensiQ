...
> **Nota de superveniência.** Os nomes dos eventos descritos neste ADR foram depois refinados, sem alterar a semântica de ledger aqui fixada. O ADR-0016 desdobrou a génese por proveniência — `APREENSAO` passou a `APREENSAO_OBJETO`/`APREENSAO_DADOS` e acrescentou-se `DERIVACAO_ITEM`; `VALIDACAO` passou a `VALIDACAO_APREENSAO`. O ADR-0017 separou a transferência de custódia nos dois tempos — `ENCAMINHAMENTO_CUSTODIA` (push, saída em trânsito, sem GPS) e `RECEPCAO_CUSTODIA` (pull, chegada e tomada de GPS). Os valores `TRANSFERENCIA_CUSTODIA` e `ASSUNCAO_CUSTODIA` mantêm-se no enum para integridade de ledgers históricos, mas já não são oferecidos como próximo evento. O código actual reflecte estes nomes (`EventType`, `src/backend/core/policy/event_states.py:41-82`), com as guardas a operar sobre conjuntos de eventos de génese (`GENESIS_EVENTS`, `SEIZURE_GENESIS_EVENTS`) em vez da guarda literal `APREENSAO_OBJETO`. A guarda `INICIO_PERICIA` ⇐ `DESPACHO_PERICIA` mantém-se. Tudo o resto deste ADR permanece válido.
>
...
>
> **Nota de superveniência (2026-06-11) — validação é ato, não estado.** A derivação descrita em «Estado legal derivado» foi refinada: `validada` **deixou de ser um estado derivado**. A validação da apreensão (CPP art. 178.º/6) é um ato jurídico que não desloca a prova — misturá-la no eixo de custódia fazia a etiqueta desaparecer quando o item viajava (a receção de regresso ao OPC re-derivava `à_guarda_OPC`). Passou a **eixo próprio**, derivado à parte: `validation_status` em `core/policy/event_states.py` devolve `por_validar`/`em_atraso`/`validada`/não-aplicável, com a exigência extinta pela disposição final (`DISPOSAL_EVENTS`). O estado derivado descreve apenas custódia/localização; a regra «existe `VALIDACAO` → `validada`» da secção de derivação fica substituída por «após `APREENSAO`, em repouso no OPC → `à_guarda_OPC`». O registo do ato faz-se por formulário próprio (quem validou, data do despacho, justificação — texto em `observations`, que integra a fórmula do hash; o `timestamp` do evento continua a ser sempre o do servidor), exigido também na fronteira da API. Tudo o resto deste ADR permanece válido.
>
> **Nota de superveniência (2026-06-11, mais tarde) — autoridade do ato certificado ESTRUTURADA (hv4).** O parágrafo anterior foi refinado: a identificação de quem profere o ato certificado (validação da apreensão — CPP art. 178.º/5-6 — e despacho para perícia — CPP art. 154.º) **deixou de viver como texto em `observations`** e passou a campos estruturados do evento — `authority_nome`/`authority_cargo`, `act_declared_at` (data declarada do ato, normalizada a UTC) e, no despacho, `act_deadline_days` (prazo fixado para a perícia, do qual se re-deriva a data-limite). Os campos entram na fórmula do hash (geração `hv4` — ver ADR-0013) e são exigidos pelo `clean()` do modelo (recusados como identidade órfã nos restantes eventos); `observations` volta a ser justificação/referência livre. Eventos históricos mantêm o texto certificado e a sua fórmula de origem.
