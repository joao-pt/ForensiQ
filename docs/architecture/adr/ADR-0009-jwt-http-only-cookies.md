...
2. **Atributos dos cookies:**
   ...
   - `Max-Age` ligado à vida do token: access **60 min**, refresh **7 dias** (configuráveis via env vars `JWT_ACCESS_TOKEN_LIFETIME_MINUTES` e `JWT_REFRESH_TOKEN_LIFETIME_DAYS`, no bloco `SIMPLE_JWT` de `forensiq_project/settings.py` — `SIMPLE_JWT['ACCESS_TOKEN_LIFETIME']` e `SIMPLE_JWT['REFRESH_TOKEN_LIFETIME']`, settings.py:198-209). A janela mais longa do refresh é compensada pela rotação automática (ver ponto 5) e pela blacklist em cada refresh.
