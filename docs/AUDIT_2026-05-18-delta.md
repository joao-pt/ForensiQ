...
**Stack (snapshot do commit `2183600`; updates de Sem.10/11 listados em §8.1; re-classificação N2 em Sem.12 via [ADR-0012](architecture/adr/ADR-0012-pdf-transport-guide.md)):** Django 6.0.5 + DRF 3.17.1 + djangorestframework-simplejwt 5.5.1 + dj-database-url 3.1.2 + gunicorn 26 + PostgreSQL (Neon) + Fly.io. Dev: pytest-django 4.12. Frontend templates Django + JS vanilla.
...
**Veredito executivo:** **8 dos 10 itens do Top-10 de abr 2026 estão resolvidos**. O hardening de segurança crítica (JWT HttpOnly, CSP nonce, hash determinístico, IDOR, TRUSTED_PROXIES) materializou-se em código, com cobertura de testes adicional (+128%, 393 testes pós-Sem.11). Em Sem.11 foram encerrados **S9** (EXIF strip — último 🟠 Alto operacional do §2), **N8** (throttle dedicado `imei_lookup`), **N11** (alinhamento CSRF/CORS) e **N14** (try/finally em PDF). Em Sem.12 (este planeamento), **N2 foi re-classificado como não-aplicável** via [ADR-0012](architecture/adr/ADR-0012-pdf-transport-guide.md) — o PDF não é prova juridicamente auto-contida, é guia de transporte físico (paralelo DHL). Persistem como arquitecturais sem v1.1: **N4** (PDF assíncrono via Celery/RQ), **P5** (PurgeCSS — viola ADR-0004). Persistem como operacionais de monitorização: **B9** (retenção AuditLog — atacado em Sem.12), **N9** (quota IMEI — atacado em Sem.12). Dois achados da auditoria de abr eram **falsos positivos** quando reverificados (B11, ip_address fallback).
...
O ForensiQ entrega com **8 dos 10 itens do Top-10 de Abril fechados**, **5 dos 5 N* 🟠 Alto fechados ou re-classificados** (N1/N3/N5/N9 fechados em 18 mai; N2 re-classificado como não-aplicável via [ADR-0012](architecture/adr/ADR-0012-pdf-transport-guide.md) em Sem.12), **S9 fechado em Sem.11** (último 🟠 Alto operacional do §2, eliminando exfiltração de EXIF), **N8/N11/N14 fechados em Sem.11**, e **trilha auditável dos restantes**. A suite de testes cresceu para **393 testes** (de 382 na auditoria de 18 mai → +11 testes em Sem.11: 2 N11 + 2 N14 + 2 N8 + 5 S9), todos a passar.
