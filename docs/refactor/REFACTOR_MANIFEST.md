...
**Estado da Fase 2 (backend) — essencialmente fechada.** Concluídos e committados:
...
- **T16** — testes (`bb7d874`+`bae98b4`): `AuditLogFactory`; `ImmutabilityTriggerTest` estende a 3.ª camada (triggers PG) a `core_evidence`/`core_occurrence` + DELETE via cursor bruto; `CsrfCookieFlowTest` exercita o `enforce_csrf` a sério (`enforce_csrf_checks=True`: POST sem token→403, com token→201); consolidação de duplicados verificada (−207 linhas, sem perda de cobertura).
- **T13** — pipeline (`3ad6f70`+`5ab68c6`): `ruff check src/backend` 100% limpo (SIM117/SIM102/B007/B905/SIM115 + import-sort); job CI `lint` (ruff fixo); job CI `test-postgres` (corre `ImmutabilityTriggerTest` contra postgres:16 — os 6 triggers deixam de fazer skip); gate de cobertura ÚNICO `fail_under=80` (real ~84%, resolve a divergência 85/75/68); trivy pinado a SHA `ed142fd` (v0.36.0). `ci_postgres_settings.py` herda `test_settings` e só troca a BD.
- **Lows de segurança (família T12):** healthcheck passa a DRF com `HealthcheckRateThrottle` (120/min por IP, scope `healthcheck`; teste 429); `JWT_SIGNING_KEY` falha-fechado em produção (`ImproperlyConfigured` se faltar com `DEBUG=False`/não-teste); CSP Report-Only sem `report-uri` **documentado como decisão** (DEBUG-only; violações já visíveis na consola; sem colector POST não-autenticado).

**Diferido dentro do T13 (commit dedicado, fora deste âmbito):** bump das versões pre-commit (ruff 0.5→0.15, black 24→26, semgrep repo migrado) + enforcement de **formatação** (ruff-format/black `--check`) + política de fim-de-linha (`.gitattributes` LF) — exige um commit "reformatar tudo" (~15 ficheiros) que se isolou de propósito.

**Falta na Fase 2 (menor):** **T11** (sinalizar `/stats/` legacy + `investigation_report` como v2 — esconder, não apagar), **T14** (doc-drift: README `293`→contagem real de testes, paleta do PDF, `.env.example`, volume no runbook de DR, RGPD Art.32) e o T13-diferido acima. **Fase 3 (frontend)** consome todo o novo backend (custódia event-ledger, prioridade 2-estados, feed, deltas, tiles CartoDB; `transition_modal.js` passa a ler `data.detail`).
